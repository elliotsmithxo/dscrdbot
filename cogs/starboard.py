...
class Starboard(commands.Cog):
    ...
    def get_channel_id(self):
        try:
            with open(CONFIG_PATH, "r") as f:
                raw = json.load(f).get("starboard_channel_id")
                return int(raw) if raw else None
        except Exception as e:
            print(f"[ERROR] Failed to load starboard_channel_id: {e}")
            return None
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        starboard_channel_id = self.get_channel_id()
        print(f"[DEBUG] Loaded starboard_channel_id: {starboard_channel_id}")

        if starboard_channel_id is None:
            return

        starboard_channel = self.bot.get_channel(starboard_channel_id)
        print(f"[DEBUG] Resolved channel: {starboard_channel}")
